...
- 商品新增／編輯／刪除：基本資料、價格、多組規格、上架中／草稿（草稿前台不顯示）
- 商品圖片：可上傳自己的照片（存 Cloudflare R2），沒上傳就用預設的精選圖庫
...
| 檔案儲存 | Cloudflare R2（商品上傳圖片） |
| 測試 | Vitest + `@cloudflare/vitest-pool-workers`（在真正的 Workers 執行環境跑，含 D1／R2 binding） |
| 部署 | Cloudflare Workers + Static Assets |

前台靜態檔案、後端 API 與 D1／R2 綁定由同一個 Worker 部署，日後可直接擴充後端路由與資料表。
...
需要 Node.js 22 以上（wrangler 4.x 的硬性要求）。

```bash
npm install
npm run db:init      # 建立本機資料庫並灌入種子資料
npm run dev           # http://localhost:8788
npm test              # 跑 functions/ 的自動化測試（vitest + Workers 執行環境）
```
...
- 30 件種子商品的照片沿用精選 Pexels 免版稅圖庫；後台可另外上傳自己的照片覆蓋
